...
`COMPARE` では、元の展開図と車体への投影を並べて確認できます。`?view=compare&wrap=Divide` のように指定すると比較画面を直接開けます。公式リポジトリに車両の3DモデルとUV頂点の対応情報は含まれないため、車体への投影は静止画上で手作業により合わせたものです。模様の位置や曲面での伸び方はTesla車内の表示とは一致しません。
